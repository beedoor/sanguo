...
public class CitySearchAndGoldTask extends GameTask {
	...
	private void msgIdCitySearch()
	{
		...
		try {
			...
			logger.info("当前金币[{}],获得金币[{}],武将[{}],物品[{}]",new Object[]{cityInfo.getGold(),cityInfo.getSearchGold(),GameUtil.ingoreNull(cityInfo.getHeros()),GameUtil.ingoreNull(cityInfo.getItems())});
		} catch (Exception e) {
			logger.error("转换异常",e);
		}
	}
