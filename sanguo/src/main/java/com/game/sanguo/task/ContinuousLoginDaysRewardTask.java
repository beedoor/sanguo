...
public class ContinuousLoginDaysRewardTask extends GameTask {
	...
	public void run() {
		try {
			msgIdRecvContinuousLoginDaysRewardTask();
		} catch (Throwable e) {
			logger.error("领取连续登录礼包异常",e);
		}
	}

	private void msgIdRecvContinuousLoginDaysRewardTask(){
		...
		postMethod.addParameter(new NameValuePair("c0-e4", "number:4"));
		...
		try {
			...
			if(null != awardInfo)
			{
				logger.info("连续登录奖励["+awardInfo.toString()+"]");
			}
		} catch (IOException e) {
