...
public class ContinuousLoginDaysRewardTask extends GameTask {
	...
	public void doAction() {
		try {
			...
			{
				sleep(10);
				logger.info("获取第几天的奖励 "+ i);
				msgIdRecvContinuousLoginDaysRewardTask(i);
			}
		} catch (Throwable e) {
	...
	private void msgIdRecvContinuousLoginDaysRewardTask(int i ){
		...
		postMethod.addParameter(new NameValuePair("c0-e4", "number:"+i));
		...
		try {
			ContinuousLoginDaysRewardInfo awardInfo = initBeanInfo(ContinuousLoginDaysRewardInfo.class, postMethod.getResponseBodyAsStream(), "dwr");
			if(null != awardInfo)
			{
				logger.info("连续登录奖励["+i+","+awardInfo.toString()+"]");
			}
		} catch (IOException e) {
